...
class NoPosEmTransformer(Module):
    def __init__(self,embedding_dim,no_heads,vocab_size,block_size,hidden_dim,no_blocks):
        self.block_size=block_size
        self.head_dim=embedding_dim//no_heads
        self.no_heads=no_heads
    ...
    def forward_prop(self,x):
        embedding=self.token_embedding.forward(x)
...
class NoLNBlock(Module):
    def __init__(self,embedding_dim,no_heads,block_size,hidden_dim):
        self.attention=MultiHeadAttention(embedding_dim,no_heads,block_size,True)
        self.ffn=FFN(embedding_dim,embedding_dim,hidden_dim)
    def forward(self,x):
        attention_scores=self.attention.forward(x,x)
        layernormed=x+attention_scores
        ff=self.ffn.forward(layernormed)
        layernormed2=ff+layernormed
        return layernormed2

class NoLNTransformer(Module):
    def __init__(self,embedding_dim,no_heads,vocab_size,block_size,hidden_dim,no_blocks):
        self.block_size=block_size
        self.head_dim=embedding_dim//no_heads
        self.no_heads=no_heads
        ...
        self.transformerblocks=[NoLNBlock(embedding_dim,no_heads,block_size,hidden_dim) for i in range(no_blocks)]
...
class NoResBlock(Module):
    ...
    def forward(self,x):
        attention_scores=self.attention.forward(x,x)
        layernormed=self.layernorm1.forward(attention_scores)
        ff=self.ffn.forward(layernormed)
        layernormed2=self.layernorm2.forward(ff)
        return layernormed2

class NoResTransformer(Module):
    def __init__(self,embedding_dim,no_heads,vocab_size,block_size,hidden_dim,no_blocks):
        self.block_size=block_size
        self.head_dim=embedding_dim//no_heads
        self.no_heads=no_heads
        ...
        self.transformerblocks=[NoResBlock(embedding_dim,no_heads,block_size,hidden_dim) for i in range(no_blocks)]
